...
describe 'where with array', {
  ...
  before-each {
    WaUser.destroy-all;
    $alice = WaUser.create({fname => 'Alice', lname => 'Anderson'});
    $bob   = WaUser.create({fname => 'Bob',   lname => 'Brown'});
    $carol = WaUser.create({fname => 'Carol', lname => 'Crane'});
    $dave  = WaUser.create({fname => 'Dave',  lname => 'Davis'});
  }
  ...
  context 'array of strings emits IN', {
    it 'returns matching count', {
      my @some = WaUser.where({fname => ['Alice', 'Carol']}).order('fname').all;
  ...
  context 'array of ints', {
    it 'finds the listed ids', {
      my @by-id = WaUser.where({id => [$alice.id, $dave.id]}).order('id').all;
  ...
  context 'where.not with array emits NOT IN', {
    it 'returns the rest count', {
      my @rest = WaUser.where.not({fname => ['Alice', 'Carol']}).order('fname').all;
  ...
  it 'where.not on empty array matches all', {
    expect(WaUser.where.not({fname => []}).count).to.eq(4);
  }

  it 'count honors IN', {
    expect(WaUser.where({fname => ['Alice', 'Bob', 'Carol']}).count).to.eq(3);
  }

  it 'IN combined with scalar where', {
    my @combo = WaUser.where({fname => ['Alice', 'Dave', 'Carol']}).where({lname => 'Davis'}).all;

    expect(@combo.elems == 1 && @combo[0].fname eq 'Dave').to.be-truthy;
  }

  it 'pluck respects IN', {
    my @names = WaUser.where({id => [$bob.id, $carol.id]}).order('fname').pluck('fname');
